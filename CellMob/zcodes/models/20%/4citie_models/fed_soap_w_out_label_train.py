...
RUN_OUTPUT_DIR = OUTPUT_ROOT / "fedavg_soap_kaust_jeddah_mekkah_kz_ovr_safe"
...
# ---------------------------------------------------------
# IMPORTANT:
# Replace *_kz_cleaned.csv below if your real filenames differ
# ---------------------------------------------------------
CITY_FILES = {
    "kaust": {
        "train": {
    ...
    "kazakhstan": {
        "train": {
            "walk": TRAIN_DIR / "walk_kz_cleaned.csv",
            "car": TRAIN_DIR / "car_kz_cleaned.csv",
        },
        "test": {
            "walk": TEST_DIR / "walk_kz_cleaned.csv",
           
            "car": TEST_DIR / "car_kz_cleaned.csv",
        },
    },
}
...
# Utilities
# =========================================================
def verify_all_files_exist(city_files):
    missing = []
    for city_name, splits in city_files.items():
        for split_name, class_dict in splits.items():
            for class_name, path in class_dict.items():
                if not path.exists():
                    missing.append((city_name, split_name, class_name, str(path)))

    if missing:
        msg = ["The following files are missing:"]
        for city_name, split_name, class_name, path in missing:
            msg.append(f"  city={city_name}, split={split_name}, class={class_name}, path={path}")
        raise FileNotFoundError("\n".join(msg))
...
def binary_has_two_classes(y_binary: np.ndarray) -> bool:
    return len(np.unique(y_binary)) >= 2
...
def evaluate_binary_model(model, X_eval, y_eval_multiclass, target_class):
    ...
    if len(np.unique(y_true_bin)) < 2:
        ap = float("nan")
        precision_curve = np.array([1.0, 1.0])
...
def federated_aploss_finetune_for_class(
    ...
    # only clients that have both positive and negative labels locally
    active_clients = [
        city_name
        for city_name, info in client_train_binary.items()
        if info["ap_loader"] is not None
    ]

    if len(active_clients) == 0:
        print(f"No eligible clients for SOAP on class {class_name}. Skipping SOAP stage.")
        elapsed = 0.0
        return global_model, elapsed, np.nan, round_loss_history

    client_sample_counts = [client_train_binary[c]["num_train"] for c in active_clients]
    sampling_rate = BEST_SAMPLING_BY_CLASS[class_name]

    print(f"gamma={AP_GAMMA}, margin={AP_MARGIN}, sampling_rate={sampling_rate}")
    print(f"Active SOAP clients for {class_name}: {active_clients}")
    ...
    for round_idx in range(1, GLOBAL_ROUNDS_AP + 1):
        ...
        for city_name in active_clients:
            local_model = BinaryRNNScorer(
                input_size=len(FEATURE_COLUMNS),
                hidden_size=HIDDEN_SIZE,
                num_layers=NUM_LAYERS,
...
def plot_pr_curves(results_by_class, save_path: Path, title: str):
    ...
    for class_name in ["walk", "bus", "car"]:
        ...
        ap_str = "nan" if np.isnan(entry["ap"]) else f"{entry['ap']:.4f}"
        plt.plot(
            entry["recall_curve"],
...
def save_metrics_summary(
    ...
    macro_auprc = float(np.nanmean([results_by_class[c]["ap"] for c in ["walk", "bus", "car"]]))
    ...
    for class_name in ["walk", "bus", "car"]:
        lines.append(f"{class_name:>5} | AP: {results_by_class[class_name]['ap']}")

    lines.append("")
    lines.append("Best validation AP after federated BCE pretraining:")
    for class_name in ["walk", "bus", "car"]:
        lines.append(f"{class_name:>5} | BCE Val AP: {best_bce_val_aps[class_name]}")

    lines.append("")
    lines.append("Best validation AP after federated APLoss/SOAP:")
    for class_name in ["walk", "bus", "car"]:
        lines.append(f"{class_name:>5} | APLoss Val AP: {best_aploss_val_aps[class_name]}")
    ...
    for city_name, info in client_info.items():
        lines.append(
            f"{city_name:>10} | train_windows={info['train_windows']} | "
...
def main():
    ...
    verify_all_files_exist(CITY_FILES)
    ...
    for target_class in [0, 1, 2]:
        ...
        for city_name in CITY_FILES.keys():
            ...
            if binary_has_two_classes(y_train_binary):
                ...
                eligible_for_ap = True
            else:
                dataset_for_ap = None
                ap_loader = None
                eligible_for_ap = False
                print(
                    f"WARNING: {city_name.upper()} has only one binary class for "
                    f"{class_name} vs rest. Skipping this client in SOAP stage."
                )
            ...
            per_city_binary[city_name] = {
                ...
                "eligible_for_ap": eligible_for_ap,
            }

        pretrained_model, bce_time_sec, best_bce_val_ap, _ = federated_bce_pretrain_for_class(
            target_class=target_class,
    ...
    print("\nAP / AUPRC per class:")
    for class_name in ["walk", "bus", "car"]:
        print(f"{class_name:>5} | AP: {results_by_class[class_name]['ap']}")

    print("\nBest validation AP after federated BCE pretraining:")
    for class_name in ["walk", "bus", "car"]:
        print(f"{class_name:>5} | BCE Val AP: {best_bce_val_aps[class_name]}")

    print("\nBest validation AP after federated APLoss/SOAP:")
    for class_name in ["walk", "bus", "car"]:
        print(f"{class_name:>5} | APLoss Val AP: {best_aploss_val_aps[class_name]}")
