...
with a small 8-configuration sweep around the best setting
==========================================================================
"""
...
def run_single_config(
    ...
    for target_class in [0, 1, 2]:
        ...
        pretrained_model, bce_time_seconds, _ = pretrain_binary_model_bce(
            X_train=X_train,
        ...
        final_model, aploss_time_seconds, _ = finetune_binary_model_aploss(
    ...
    row = {
        ...
        "bce_time_total_sec": bce_times["walk"] + bce_times["bus"] + bce_times["car"],
        "aploss_time_total_sec": aploss_times["walk"] + aploss_times["bus"] + aploss_times["car"],
        "grand_total_sec": (
            bce_times["walk"] + bce_times["bus"] + bce_times["car"] +
            aploss_times["walk"] + aploss_times["bus"] + aploss_times["car"]
        ),
    }
    ...
    print(f"  Total time(s): {row['grand_total_sec']:.2f}")

    return row, models, val_results_by_class
...
def main():
    ...
    total_configs = len(GAMMA_LIST) * len(MARGIN_LIST) * len(CAR_SAMPLING_LIST)
    ...
    for gamma, margin, car_sampling in product(
        GAMMA_LIST,
        MARGIN_LIST,
        CAR_SAMPLING_LIST,
    ):
        ...
        row, models, _ = run_single_config(
            config_id=config_id,
            gamma=gamma,
            margin=margin,
            walk_sampling=FIXED_WALK_SAMPLING,
            bus_sampling=FIXED_BUS_SAMPLING,
            car_sampling=car_sampling,
            X_train=X_train,
            y_train=y_train,
            X_val=X_val,
    ...
    results_csv_path = OUTPUT_DIR / "pretrain_aploss_sweep8_results.csv"
    ...
    print("\nAll configurations by validation Macro AUPRC:")
    ...
    pr_plot_path = OUTPUT_DIR / "pr_curves_pretrain_aploss_best8_test.png"
    plot_pr_curves(
        best_test_results_by_class,
        pr_plot_path,
        title="Best 8-Config BCE Pretrain + APLoss Fine-Tune PR Curves on Test",
    )
