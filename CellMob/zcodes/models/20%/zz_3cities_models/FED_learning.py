...
RUN_OUTPUT_DIR = OUTPUT_ROOT / "fedavg_kaust_jeddah_mekkah_rnn_ce_corrected_round30"
...
def fit_global_standardizer(city_files):
    """
    Fit ONE shared scaler from all TRAINING ROWS across all cities/classes.
    This avoids client-specific or class-specific scaling inconsistency.
    """
    frames = []

    for city_name, split_files in city_files.items():
        for class_name, path in split_files["train"].items():
...
def fedavg_weighted_aggregate(local_state_dicts, client_names, client_sample_counts, verbose=True):
    """
    Weighted FedAvg:
    global parameter = sum_k (n_k / total_n) * local_parameter_k
    where n_k = number of local training windows for client k.
    """
...
def save_metrics_summary(results, round_loss_history, client_info, global_class_weights, save_path: Path):
    ...
    lines.append("")
    lines.append("Average local train loss per global round:")
...
def main():
    ...
    # Shared global standardization statistics from ALL training rows only
    ...
    for city_name, split_files in CITY_FILES.items():
        ...
        X_train_city, y_train_city, _ = build_dataset_from_files(
    ...
    # Shared global class weights from ALL training windows
    ...
    print(f"\nShared global class weights from ALL TRAINING WINDOWS: {global_class_weights.tolist()}")
    ...
    print(f"\nStarting corrected federated training on {DEVICE} ...")
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        ...
        for city_name in CITY_FILES.keys():
            ...
            # Start each client from the current global model
            ...
            # SAME global class weights for all clients
        ...
        # Weighted FedAvg by number of training windows
        ...
        print(f"\nRound {round_idx:02d} average local loss across clients: {round_avg_loss:.6f}")
    ...
    model_path = RUN_OUTPUT_DIR / "fedavg_global_rnn_kaust_jeddah_mekkah_corrected.pth"
